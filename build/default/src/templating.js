import{html}from"../node_modules/lit-element/lit-element.js";import{styleMap}from"../node_modules/lit-html/directives/style-map.js";import{unsafeHTML}from"../node_modules/lit-html/directives/unsafe-html.js";export const loadingTemplate=()=>html`
  loading ...
`;export const baseArtworkLayersTemplate=({useHiResAssets,protoId})=>{return html`
  ...
  `};export const mythicImageLayersTemplate=({type,resolutionSettings,qualityName})=>{return html`
  ...
  `};export const nonMythicImageLayersTemplate=({type,resolutionSettings,qualityName,rarity,god,set})=>html`
    <picture class="card__baseLayer">
      <source
    ...
    ${"core"===set?html`
        ...
        `:null}
  `;export const textLayersTemplate=({mana,name,effect,attack,health,type,ch,cw})=>{const isACreatureOrWeapon=RegExp(/creature|weapon/).test(type),crammedTextMode=14<=effect.split(" ").length;return html`
    <div
      class="card__manaText"
      style=${styleMap({fontSize:`${10.5*ch}px`,top:`${5.5*ch}px`,left:`${9.5*cw}px`,width:`${19*cw}px`})}
    >
      ${mana}
    </div>
    <div
      class="card__nameText"
      style=${styleMap({fontSize:`${4.93*ch}px`,bottom:`${35*ch}px`,left:`${12*cw}px`,right:`${5*cw}px`})}
    >
      ${name}
    </div>
    ...
    <div
      ...
      style=${styleMap({fontSize:`${crammedTextMode?3.8*ch:4*ch}px`,lineHeight:crammedTextMode?1.1:1.3,bottom:`${7.75*ch}px`,height:`${22.5*ch}px`,left:`${20*cw}px`,right:`${12*cw}px`})}
    >
    ...
    ${isACreatureOrWeapon?html`
          <div
            class="card__attackText"
            style=${styleMap({fontSize:`${9.5*ch}px`,bottom:`${3.5*ch}px`,width:`${15*cw}px`,left:`${10.5*cw}px`})}
          >
            ${attack}
          </div>
          ...
          <div
            ...
            style=${styleMap({fontSize:`${9.5*ch}px`,bottom:`${3*ch}px`,width:`${15*cw}px`,right:`${"weapon"===type?.8*cw:4.5*cw}px`})}
          >
            ${health}
          </div>
        `:null}
